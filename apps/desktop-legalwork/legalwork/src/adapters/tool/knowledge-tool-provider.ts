  ...
  return [{
    ...
    tools: [
      ...
      LocalToolHost.defineTool({
        ...
        description: 'Create a new folder under managed root. Use this to organize documents into categories (e.g. "合同审查", "法规汇编", "案例研究"). Parent folders are auto-created.',
        inputSchema: {
          type: 'object',
          properties: {
        ...
        policy: 'auto',
        execute: async (args) => {
          const folderPath = typeof args.path === 'string' && args.path.trim() ? args.path.trim() : ''
