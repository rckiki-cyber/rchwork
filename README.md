...
- **设置新增「模型配置」独立子项**：认证方式（API Key / ChatGPT 账号）、模型供应商、API Key、Base URL、接口协议、模型列表、当前模型统一管理，从「通用」与「AI 助手」中抽出集中展示。
- **支持 ChatGPT 账号认证**：设置里可选「ChatGPT 账号」登录（走 ChatGPT 订阅，无需 API Key），主页模型自动联动账号可用模型。
- **中转站（Relay）适配**：接口协议可选 OpenAI Chat Completions / OpenAI Responses / Anthropic Messages；`thinking` 字段按 host 收敛，仅在官方 DeepSeek 域名自动注入，中转站不再误带；费用估算按 host 收敛，中转站不再按官方价乱估。
- **对话文件列表改悬浮层**：默认不自动弹出，点击顶栏「对话文件」按钮才弹出悬浮列表，点具体文件后才打开右侧栏预览。
- **对话文件优化**：同一文件不再重复显示（用户上传 + Agent 产出按文件名去重）；过滤日期/域名等伪文件条目；图片附件显示格式角标；预览支持更多文本/代码格式。
- **知识库文件预览修复**：对话中引用的知识库文件（如 `论文/xxx.pdf`）不再报 File not found，自动降级走知识库 API 读取。
- **法律调研稳定性修复**：规划后不再卡住（SSE 事件批处理 + 前端节流 + 超时兜底）；规划不再跑进「调研总结」；仅生成规划未检索时不再误报「完成」；阶段播报编号按内容阶段号对齐；调研记录不再污染主页 agent 对话（独立工作区）。
- **Bug 修复**：认证方式切换（ChatGPT ↔ API Key）失效；代码审查发现的 SSE 批处理时序、连接状态竞态、调研中止丢文本、报告标题误匹配等问题。
