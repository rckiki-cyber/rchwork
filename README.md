...
- **Agent 运行时稳定性全面加固**：修复 SSE 首连临界超时、Runtime 重启/瞬时断连误判失败、学习迭代重复 JSON 输出、文档 Skill 启动竞态等问题，长任务和连续工具调用更稳。
- **法律调研更准确**：国家法律法规数据库会优先识别并检索明确法律名称，例如“民法典第五百八十五条”优先命中《中华人民共和国民法典》，减少模糊检索跑偏；明确说“不需要联网检索”时也会真正停止强制联网。
- **文书写作事实边界加固**：只把用户、附件或已核验来源明确提供的内容写成确定事实，缺失的验收、催告、质量异议、金额、日期等不再自行补写。
- **Filesystem MCP 改为本地离线启动**：默认文件系统 MCP 不再依赖每次启动时通过 npx 联网拉取，降低 npm registry 波动造成的连接超时和 `Connection closed`。
- **插件市场状态更清晰**：区分“网络环境不可用”和真正的认证/配置错误，Context7、Playwright 等可恢复状态不再制造红色故障噪音。
- **多项跨平台与错误治理修复**：包括 Windows Office Python 运行环境、bash session 丢失、read-before-edit 预期错误、GPU/Utility 子进程崩溃噪音、知识库树异常恢复等。
- 完整变更见下方「🔄 更新记录 → v0.3.29」。
...
- **SSE 首连超时根因修复**：服务端在流式响应建立后立即 flush headers，不再让 15 秒 heartbeat 与 15 秒客户端启动超时卡在同一临界点，避免偶发 `sse start timeout`。
- **Runtime 瞬时断连可恢复**：学习迭代遇到 `fetch failed`、连接重置或 Runtime 重启时保留当前轮次并等待恢复，不再直接写入失败记录或上报错误；轮询阶段允许有限次数的瞬时断连恢复。
- **学习迭代 JSON 容错增强**：模型在结果标记内输出“一个有效 JSON + 重复/尾随 JSON”时可提取首个完整对象，不再因 `Unexpected non-whitespace character after JSON` 直接失败。
- **文档 Skill 启动竞态修复**：`document_skill_execute` 在延迟发现 Skill 尚未完成时会主动 refresh 后重试，避免启动早期误报 `legal-document-formatting Skill is unavailable`。
- **法律调研精确命中法名**：国家法律法规数据库检索会先从问题中提取明确法律名称并优先检索规范化标题，例如“民法典第五百八十五条”先查《中华人民共和国民法典》，再进行条文/关键词扩展，降低海商法等无关候选干扰。
- **尊重“不需要联网检索”**：事实核验的联网退出规则补齐“不需要联网/搜索/检索/查询”等自然表达，用户明确要求不联网时不再被强制 web search。
- **文书事实边界加固**：文书模型请求新增明确约束，只能把用户、附件或工具核验过的内容写成确定事实；验收、签收、质量异议、催告沟通、付款条件、日期、损失金额等缺失信息必须使用“待确认”或条件式表述，并在终稿前复核主体、金额和时间。
- **Filesystem MCP 离线化**：把默认 `npx @modelcontextprotocol/server-filesystem` 改写为随 LegalWork 一起构建的本地 MCP 入口，避免每次启动依赖 npm registry，降低网络波动导致的 MCP 连接超时。
- **插件市场状态治理**：Context7、Playwright 等 lenient MCP 的可恢复异常不再计入顶部红色错误；GitHub MCP 的网络类失败显示为“需要网络环境”，但认证错误仍保留为真实错误，减少误报同时不掩盖配置问题。
- **测试不再污染用户配置**：桌面 Runtime 测试统一使用每个测试独立的临时 data dir，不再写入真实 `~/.legalwork/legalwork/config.json`。
- **跨平台与运行时错误治理**：修复 Windows Office Python `PYTHONHOME`/解释器路径问题、bash session 丢失与 read-before-edit 预期错误噪音、知识库树异常恢复，并对可自动恢复的 GPU/Utility 子进程崩溃做 burst 去噪，仅连续异常时上报。
