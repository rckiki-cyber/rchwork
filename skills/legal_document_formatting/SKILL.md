...
**在正式开始起草裁判文书前，必须先调用 `request_document_preferences` 工具收集用户偏好。** 不得跳过此步骤。

### 操作步骤

1. 在阅读案卷材料并判断民事/刑事后，调用 `request_document_preferences` 工具并传入 `documentType: "judgment"`。
2. 等待用户在弹出的选项卡片中完成选择。
3. 将用户返回的偏好结果作为裁判文书起草的约束条件， particularly 在说理详略、案例引用和格式严格度上。
4. 裁判文书格式受法定规范约束，偏好选择仅在法定框架内调整说理详略和案例引用策略。
