...
- 新增几何锁定模板实例化脚本，只允许替换 `value` 占位符；缺失值、未知值、残留占位符或实例化后领域校验失败时阻断输出。
- 新增复杂案件三分区模板及去标识证券虚假陈述代表性输入，容量不足时要求拆主图与附图。
- 新增重叠、文字溢出、长 edge 标签三个最小违规反例，以及合法父子容器、背景框包含和边界相接近似正例。

### 修复

- 修复校验器把合法容器子节点误判为非法 parent 的问题，并校验子节点相对坐标是否超出容器。
- 修复校验器无法发现节点正面积重叠、中文文本宽高溢出和超长 edge 标签的问题。
- 为房地产平面图背景框补充显式容器语义，避免合法包含被误报为重叠。

### 改进

- 有适配模板时强制锁定几何，仅填内容；无适配模板才允许新写 XML，并必须通过领域门禁。
- 明确复杂案件默认拆主图与附图；同图包含时间、计算、程序等多类信息时使用有容量约束的分区模板。
- 导出脚本在复制源文件和调用 draw.io 前重跑领域校验，错误产物不进入交付目录。

### 技术优化

- 容器感知碰撞算法排除真实祖先/后代与显式容器完整包含，仅阻断其他正面积相交；边界相接合法。
- 文本预检同时估算中英文混排宽度、显式/自动换行、字体、padding 和所需高度；临界风险告警，确定溢出报错。
- 8 项领域回归与 19 个发布模板全检通过；19/19 模板经 draw.io CLI 成功导出非空 PNG，并完成全量联系表及长标签风险图目视检查。

### 待办事项

- 原始证券虚假陈述案件材料与旧 `.drawio` 未随本轮提供，保留原始案例端到端复验任务，不以代表性样例冒充完成。
